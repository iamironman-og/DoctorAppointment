$(function(){
	...
	$('select[name="filter_category"]').add('input[name="filter_appointmentDate"]').add('input[name="filter_doctorName"]').add('input[name="filter_bookingId"]').add('input[name="filter_bookingDate"]').add('select[name="filter_bookingStatus"]').on('change keyup',function(){
		$.fn.displayList();
	});
	...
	$('#reset_bookingId').click(function(){
		$('input[name="filter_bookingId"]').val('');
		$.fn.displayList();
	});
	...
	$('#display-bookings').on('click','a.prescription',function(e){
		...
		var parent=$(this).parents('tr');
		var bid=parent.find('input[type=hidden].booking-id').val();
		var form=$('<form action="downloadprescriptionFile.php" method="POST"></form>');
